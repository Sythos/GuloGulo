...
import { detectBrowserTimeZone, formatLocalTimestamp, formatMessageTimestamp, normalizeTimeZone, resolveViewerTimeZone, TimezoneError } from './timezone.ts';

test('detects and resolves deterministic browser/manual time zones', () => {
  const intl = { DateTimeFormat: () => ({ resolvedOptions: () => ({ timeZone: 'Europe/Rome' }) }) } as unknown as typeof Intl;
  ...
  assert.throws(() => normalizeTimeZone('Not/AZone'), (error: unknown) => error instanceof TimezoneError && error.code === 'INVALID_TIMEZONE');
});
