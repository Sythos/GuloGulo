...
test('upgrade request rejects same targets and unsafe execution inputs', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => (error as { code?: string }).code === 'SAME_SOURCE_AND_TARGET',
  );
  assert.throws(
    () => createUpgradeRequest({ ...TARGET, platform: 'shell' }),
    (error: unknown) => (error as { code?: string }).code === 'INVALID_PLATFORM',
  );
  assert.throws(
    () => createUpgradeRequest({ ...TARGET, idempotencyKey: '../docker socket' }),
    (error: unknown) => (error as { code?: string }).code === 'INVALID_CONFIGURATION',
  );
});
...
test('Docker replacement shares external state and never copies mailbox data', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => (error as { code?: string }).code === 'STATE_REFERENCE_FORBIDDEN',
  );
  assert.throws(
    () => createDockerReplacementPlan({ sourceDigest: SOURCE_DIGEST, targetDigest: TARGET_DIGEST, queue: { persistent: false } }),
    (error: unknown) => (error as { code?: string }).code === 'QUEUE_NOT_PERSISTENT',
  );
});
...
test('Kubernetes plan enforces readiness, PDB, external state, and bounded drain', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => (error as { code?: string }).code === 'READINESS_POLICY_VIOLATION',
  );
});
...
test('allowlisted actions contain no raw shell or kubectl command', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => (error as { code?: string }).code === 'ACTION_NOT_ALLOWLISTED',
  );
});
...
test('provider control plane is idempotent and tenant roles are denied', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => (error as { code?: string }).code === 'TENANT_CONTROL_PLANE_FORBIDDEN',
  );
...
test('provider control plane enforces preflight, prepare, cutover, rollback, and finalize gates', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => (error as { code?: string }).code === 'PREFLIGHT_REQUIRED',
  );
  assert.throws(
    () => controller.preflight({ actor: ACTOR, operationId: planned.operationId, evidence: { ...allPreflight(), greenReady: false } }),
    (error: unknown) => (error as { code?: string }).code === 'PREFLIGHT_FAILED',
  );
  ...
  assert.throws(
    ...
    (error: unknown) => (error as { code?: string }).code === 'GREEN_NOT_READY',
  );
...
test('preflight evidence defaults closed and requires every gate', () => {
  ...
  assert.throws(
    () => createPreflightEvidence({ imageVerified: 'true' }),
    (error: unknown) => (error as { code?: string }).code === 'INVALID_CONFIGURATION',
  );
