...
function createAuditEvent({ event, account, actor, occurredAt, operationId, metadata = {} }: { event: string; account: AccountRecord; actor: { actorId: string; role: string }; occurredAt: unknown; operationId?: string; metadata?: UnknownRecord }): AccountAuditEvent {
  return deepFreeze({ schemaVersion: ACCOUNT_LIFECYCLE_SCHEMA_VERSION, event, occurredAt: iso(occurredAt, 'occurredAt'), tenantId: account.tenantId, userId: account.userId, actorId: actor.actorId, actorRole: actor.role, ...(operationId ? { operationId } : {}), metadata }) as AccountAuditEvent;
}
...
export function createAccountLifecycleStore({ now = () => new Date(), recoveryDays = DEFAULT_ACCOUNT_RECOVERY_DAYS }: { now?: () => DateInput; recoveryDays?: number } = {}): AccountLifecycleStore {
  ...
  const result = (operation: string, account: AccountRecord, audit?: AccountAuditEvent, extra: UnknownRecord = {}): AccountOperation => deepFreeze({ schemaVersion: ACCOUNT_LIFECYCLE_SCHEMA_VERSION, operation, account: publicAccount(account), ...(audit ? { audit } : {}), ...extra }) as AccountOperation;
