...
export class CalDavError extends Error {
  constructor(message, code = 'CALDAV_ERROR', status = 400, details = undefined) {
...
function requiredPattern(value, pattern, field, code = 'INVALID_REQUEST') {
...
function canonicalUserId(value, field = 'userId') {
  return requiredPattern(value, USER_ID_PATTERN, field, 'INVALID_USER');
}

function canonicalCalendarSlug(value, field = 'calendarId') {
...
function isoDate(value, field = 'timestamp') {
...
function canonicalCalendarKey(ownerUserId, collectionId) {
  return `${ownerUserId}/${collectionId}`;
}

function splitCalendarId(value) {
...
function decodeToken(value, { tenantId, ownerUserId, collectionId }) {
...
function makeToken(tenantId, ownerUserId, collectionId, revision) {
...
function canonicalLineEndings(icalText) {
...
function parseCalendarComponents(icalText) {
...
function oneProperty(component, name, required = false) {
...
function assertDateValue(property, field) {
...
function parseAddress(property, field) {
  if (!property) return null;
...
function calendarMetadata(root) {
  ...
  const events = root.children.filter((component) => component.name === 'VEVENT');
  ...
  for (const component of [root, ...root.children, ...root.children.flatMap((child) => child.children)]) {
...
function publicCollection(collection, actor) {
...
function publicObject(collection, object, includeContent = true) {
  const result = {
...
function permissionFor(collection, actor) {
...
function assertPermission(collection, actor, permission) {
...
function collectionEtag(collection) {
 ...
 */
export function createCalDavStore({ tenantId, clock = () => new Date(), audit = null } = {}) {
  ...
  function emitAudit(eventType, actor, collection, details = {}) {
  ...
  function resolveCollection(actor, requestedCalendarId) {
  ...
  function recordChange(collection, objectId, object, deleted) {
  ...
  function createCalendarCollection(actor, {
  ...
  function listCalendarCollections(actor) {
    const normalizedActor = normalizeActor(actor, canonicalTenant);
  ...
  function getCalendarCollection(actor, requestedCalendarId) {
    const { actor: normalizedActor, collection } = resolveCollection(actor, requestedCalendarId);
    return publicCollection(collection, normalizedActor);
  }

  function setCalendarAcl(actor, { calendarId, delegateUserId, permissions = ['read'] } = {}) {
  ...
  function revokeCalendarAcl(actor, { calendarId } = {}) {
    const { actor: normalizedActor, collection } = resolveCollection(actor, calendarId);
    if (normalizedActor.userId !== collection.ownerUserId) fail('only the calendar owner may change its ACL', 'ACL_OWNER_REQUIRED', 403);
    collection.acl = null;
    collection.updatedAt = now();
  ...
  function getCalendarEtag(actor, requestedCalendarId) {
    const { actor: normalizedActor, collection } = resolveCollection(actor, requestedCalendarId);
  ...
  function createCalendarObject(actor, { calendarId, objectId, ical, ifNoneMatch } = {}) {
    const { actor: normalizedActor, collection } = resolveCollection(actor, calendarId);
  ...
  function getCalendarObject(actor, { calendarId, objectId } = {}) {
  ...
  function updateCalendarObject(actor, { calendarId, objectId, ical, ifMatch } = {}) {
    const { actor: normalizedActor, collection } = resolveCollection(actor, calendarId);
    assertPermission(collection, normalizedActor, 'write');
    const id = canonicalCalendarSlug(objectId, 'objectId');
    const existing = collection.objects.get(id);
  ...
  function deleteCalendarObject(actor, { calendarId, objectId, ifMatch } = {}) {
    const { actor: normalizedActor, collection } = resolveCollection(actor, calendarId);
    assertPermission(collection, normalizedActor, 'write');
    const id = canonicalCalendarSlug(objectId, 'objectId');
    const existing = collection.objects.get(id);
  ...
  function listCalendarObjects(actor, { calendarId, syncToken } = {}) {
    const { collection } = resolveCollection(actor, calendarId);
  ...
  function deleteCalendarCollection(actor, { calendarId, ifMatch } = {}) {
    const { actor: normalizedActor, collection } = resolveCollection(actor, calendarId);
  ...
  return freezeObject({
    ...
    makeSyncToken: ({ calendarId, revision: requestedRevision = revision } = {}) => {
