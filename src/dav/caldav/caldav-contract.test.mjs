...
import { CalDavError, createCalDavStore, validateICalendar } from './caldav-contract.mjs';
...
function event({ summary = 'Team meeting', uid = 'event-1@example.test', timeZone = null, method = null, attendee = null, sequence = null } = {}) {
...
test('iCalendar parser rejects malformed objects and undefined timezones', () => {
  assert.throws(() => validateICalendar('BEGIN:VCALENDAR\nVERSION:2.0\nEND:VCALENDAR\n'), (error) => error.code === 'INVALID_ICALENDAR');
  const noTimeZoneDefinition = event({ timeZone: 'Europe/Rome' }).replace(/\r\nBEGIN:VTIMEZONE\r\nTZID:Europe\/Rome\r\nBEGIN:STANDARD[\s\S]*?END:VTIMEZONE/u, '');
  assert.throws(() => validateICalendar(noTimeZoneDefinition), (error) => error.code === 'TIMEZONE_UNDEFINED');
  assert.throws(() => validateICalendar(event({ attendee: 'https://example.test/person' })), (error) => error.code === 'INVALID_ATTENDEE');
  assert.throws(() => validateICalendar(event({ method: 'UNKNOWN' })), (error) => error.code === 'UNSUPPORTED_METHOD');
});
...
test('date-only and floating local values retain their explicit timezone semantics', () => {
  ...
  assert.throws(() => validateICalendar(event().replace('DTSTART:20260822T120000Z', 'DTSTART;TZID=Europe/Rome;VALUE=DATE:20260822')), (error) => error.code === 'INVALID_TIMEZONE');
});
...
test('calendar collection is tenant and user scoped', () => {
  ...
  assert.deepEqual(store.listCalendarCollections(owner).map((item) => item.calendarId), ['alice/personal']);
  assert.deepEqual(store.listCalendarCollections(delegate), []);
  assert.throws(() => store.listCalendarCollections(otherTenant), (error) => error.code === 'CROSS_TENANT_DENIED');
  assert.throws(() => store.createCalendarCollection({ ...owner, userId: 'bob' }, { collectionId: 'other', ownerUserId: 'alice' }), (error) => error.code === 'ACL_DENIED');
  assert.throws(() => store.listCalendarCollections({ tenantId: 'acme', userId: 'alice', role: 'tenant_master' }), (error) => error.code === 'CONTENT_SCOPE_REQUIRED');
});
...
test('calendar owner can delegate one colleague with explicit ACL', () => {
  ...
  assert.deepEqual(store.listCalendarCollections(delegate).map((item) => item.permissions), [['read', 'write']]);
  assert.throws(() => store.setCalendarAcl(delegate, { calendarId: 'alice/personal', delegateUserId: 'carol', permissions: ['read'] }), (error) => error.code === 'ACL_OWNER_REQUIRED');
  assert.throws(() => store.getCalendarCollection({ tenantId: 'acme', userId: 'master', role: 'tenant_master' }, 'alice/personal'), (error) => error.code === 'CONTENT_SCOPE_REQUIRED');
  store.revokeCalendarAcl(owner, { calendarId: 'alice/personal' });
  assert.throws(() => store.getCalendarCollection(delegate, 'alice/personal'), (error) => error.code === 'ACL_DENIED');
});
...
test('conditional create returns a stable ETag and canonical object', () => {
  ...
  assert.throws(() => store.createCalendarObject(owner, { calendarId: 'alice/personal', objectId: 'event-1.ics', ical: event(), ifNoneMatch: '*' }), (error) => error.code === 'PRECONDITION_FAILED' && error.status === 412);
  assert.throws(() => store.createCalendarObject(owner, { calendarId: 'alice/personal', objectId: 'event-1.ics', ical: event() }), (error) => error.code === 'OBJECT_EXISTS');
});

test('ETags are scoped and audits contain metadata without calendar content', () => {
  const audits = [];
  const store = createCalDavStore({ tenantId: 'acme', clock: createClock(), audit: (entry) => audits.push(entry) });
  store.createCalendarCollection(owner, { collectionId: 'personal' });
  const created = store.createCalendarObject(owner, { calendarId: 'alice/personal', objectId: 'event-1.ics', ical: event(), ifNoneMatch: '*' });
  ...
  assert.equal(audits.some((entry) => entry.eventType === 'caldav.object.created'), true);
  const objectAudit = audits.find((entry) => entry.eventType === 'caldav.object.created');
...
test('conditional update requires the current ETag and preserves UID', () => {
  ...
  store.createCalendarCollection(owner, { collectionId: 'personal' });
  const created = store.createCalendarObject(owner, { calendarId: 'alice/personal', objectId: 'event-1.ics', ical: event(), ifNoneMatch: '*' });
  assert.throws(() => store.updateCalendarObject(owner, { calendarId: 'alice/personal', objectId: 'event-1.ics', ical: event({ summary: 'Updated' }) }), (error) => error.code === 'PRECONDITION_REQUIRED' && error.status === 428);
  assert.throws(() => store.updateCalendarObject(owner, { calendarId: 'alice/personal', objectId: 'event-1.ics', ical: event({ summary: 'Updated' }), ifMatch: '"stale"' }), (error) => error.code === 'PRECONDITION_FAILED' && error.status === 412);
  ...
  assert.throws(() => store.updateCalendarObject(owner, { calendarId: 'alice/personal', objectId: 'event-1.ics', ical: event({ uid: 'different@example.test' }), ifMatch: '*' }), (error) => error.code === 'UID_IMMUTABLE');
});
...
test('sync-token exposes deterministic changes and deletion tombstones', () => {
  ...
  assert.deepEqual(afterCreate.objects.map((item) => item.objectId), ['event-1.ics']);
  ...
  assert.deepEqual(afterUpdateSync.objects.map((item) => item.metadata.summary), ['Updated']);
  ...
  assert.throws(() => store.listCalendarObjects(owner, { calendarId: 'alice/work', syncToken: afterDeleteSync.syncToken }), (error) => error.code === 'SYNC_SCOPE_DENIED');
  assert.throws(() => store.listCalendarObjects(owner, { calendarId: 'alice/personal', syncToken: 'https://gulogulo.invalid/caldav/other/alice/personal/sync/1' }), (error) => error.code === 'CROSS_TENANT_DENIED');
});
...
test('read delegation cannot mutate and write delegation can mutate without owner leakage', () => {
  ...
  assert.throws(() => store.updateCalendarObject(delegate, { calendarId: 'alice/personal', objectId: 'event-1.ics', ical: event({ summary: 'Nope' }), ifMatch: created.etag }), (error) => error.code === 'ACL_DENIED');
  assert.throws(() => store.deleteCalendarObject(delegate, { calendarId: 'alice/personal', objectId: 'event-1.ics', ifMatch: created.etag }), (error) => error.code === 'ACL_DENIED');
  assert.throws(() => store.getCalendarObject(otherTenant, { calendarId: 'alice/personal', objectId: 'event-1.ics' }), (error) => error.code === 'CROSS_TENANT_DENIED');
});
...
test('collection deletion is conditional and never silently drops objects', () => {
  ...
  assert.throws(() => store.deleteCalendarCollection(owner, { calendarId: 'alice/personal' }), (error) => error.code === 'CALENDAR_NOT_EMPTY');
  const freshStore = createStore();
  const empty = freshStore.createCalendarCollection(owner, { collectionId: 'personal' });
  assert.throws(() => freshStore.deleteCalendarCollection(owner, { calendarId: 'alice/personal', ifMatch: '"stale"' }), (error) => error.code === 'PRECONDITION_FAILED');
...
test('public contract remains immutable and errors carry WebDAV status codes', () => {
  ...
  assert.throws(() => store.getCalendarObject(owner, { calendarId: 'alice/personal', objectId: 'missing.ics' }), (error) => error instanceof CalDavError && error.status === 404);
});
