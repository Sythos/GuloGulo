...
test('keeps POP3S absent unless explicitly enabled', () => {
  ...
  assert.throws(() => resolveDiscoveryService(disabled, 'pop3s'), (error) => error.code === 'SERVICE_DISABLED');
...
test('fails closed for cross-tenant and unsafe well-known requests', () => {
  const contract = createContract();
  assert.throws(() => getWellKnownResource(contract, WELL_KNOWN_PATHS.discovery, { tenantId: 'other' }), (error) => error.code === 'TENANT_MISMATCH');
...
test('rejects attempts to re-enable a disabled service through a manual override', () => {
  assert.throws(() => createContract({ manualOverrides: { pop3s: { enabled: true, host: 'mail.example.test' } } }), (error) => error.code === 'SERVICE_DISABLED');
});
