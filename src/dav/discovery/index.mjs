...
function discoveryError(message, code = 'DISCOVERY_ERROR') {
  const error = new Error(`Discovery error: ${message}`);
  error.code = code;
  return error;
}
...
function normalizeDomain(value, field = 'domain') {
...
function normalizeUsername(value, field = 'username') {
...
function normalizeTls(input, defaultMode, field = 'tls') {
...
function normalizeOrigin(origin, domain) {
...
function endpointUrl(endpoint, service) {
...
function normalizeEndpoint(service, input = {}, baseHost, { partial = false, fieldPrefix = service } = {}) {
...
function mergeEndpoint(base, override, service, baseHost) {
...
function assertDiscoveryContract(contract) {
  ...
  for (const service of SERVICE_NAMES) {
    ...
    for (const [mapName, endpoint] of [['services', contract.services[service]], ['manualOverrides', contract.manualOverrides[service]]]) {
      ...
      if (endpoint.service !== service || endpoint.protocol !== SERVICE_DEFINITIONS[service].protocol) {
...
function assertTenantMatch(contract, tenantId) {
...
export function createDiscoveryContract({
  ...
  const serviceMap = {};
  const overrideMap = {};
  const explicitOverrideServices = [];
  for (const service of SERVICE_NAMES) {
...
export function resolveDiscoveryService(contract, service, { manualOverride = null } = {}) {
...
function publicDiscoveryDocument(contract, tenantId) {
...
export function buildDiscoveryDocument(contract, { tenantId } = {}) {
  assertDiscoveryContract(contract);
...
function socketType(endpoint) {
  return endpoint.tlsMode === 'implicit' ? 'SSL' : 'STARTTLS';
}

function autoconfigXml(contract, tenantId) {
  assertTenantMatch(contract, tenantId);
 ...
 */
export function getWellKnownResource(contract, path, { tenantId } = {}) {
  assertDiscoveryContract(contract);
  assertTenantMatch(contract, tenantId);
  ...
  if (!Object.values(WELL_KNOWN_PATHS).includes(path)) throw discoveryError('well-known resource is not supported', 'NOT_FOUND');
