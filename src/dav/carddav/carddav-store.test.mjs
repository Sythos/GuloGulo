...
function aliceCard({ fullName = 'Alice Example', email = 'alice@example.test', note = '' } = {}) {
  return [
    'BEGIN:VCARD',
    'VERSION:4.0',
    'UID:alice-1',
...
function createAliceBook(store) {
...
test('address books are persisted and listed only inside the exact tenant/user scope', () => {
  ...
  assert.throws(
    ...
    (error) => error instanceof CardDavError && error.code === 'SCOPE_ROLE_DENIED' && error.status === 403,
  );
  assert.throws(
    () => store.listAddressBooks({ scope: { tenantId: 'acme', userId: 'alice', targetUserId: 'bob' } }),
    (error) => error instanceof CardDavError && error.code === 'SCOPE_TARGET_DENIED' && error.status === 403,
  );
  assert.throws(
    () => store.getAddressBook({ scope: bob, addressBookId: 'personal' }),
    (error) => error instanceof CardDavError && error.code === 'ADDRESS_BOOK_NOT_FOUND' && error.status === 404,
  );
});
...
test('vCard validation normalizes CRLF/folding and exposes safe metadata', () => {
  ...
  assert.throws(
    ...
    (error) => error.code === 'VCARD_UID_INVALID',
  );
  assert.throws(
    () => validateVCardObject('BEGIN:VCARD\r\nVERSION:2.1\r\nUID:legacy\r\nFN:Legacy\r\nEND:VCARD'),
    (error) => error.code === 'VCARD_VERSION_INVALID',
  );
});
...
test('contact create/update/delete enforce conditional ETags', () => {
  ...
  assert.throws(
    ...
    (error) => error.code === 'CONTACT_EXISTS' && error.status === 412,
  );
  assert.throws(
  ...
  assert.throws(
    ...
    (error) => error.code === 'ETAG_MISMATCH' && error.status === 412,
  );
  time.advance(1000);
...
test('conditional PUT can create a missing object and keeps UID immutable', () => {
  ...
  assert.throws(
    ...
    (error) => error.code === 'VCARD_UID_IMMUTABLE',
  );
});
...
test('metadata export never includes vCard content or identity/session data', () => {
  ...
  assert.throws(
    ...
    (error) => error.code === 'ADDRESS_BOOK_NOT_FOUND' && error.status === 404,
  );
});
