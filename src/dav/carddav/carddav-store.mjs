...
export class CardDavError extends Error {
  constructor(message, code = 'CARD_DAV_ERROR', status = 400) {
...
function cardDavError(message, code = 'CARD_DAV_ERROR', status = 400) {
  return new CardDavError(message, code, status);
}

function assertString(value, field, { pattern, maxLength = 1024, allowEmpty = false } = {}) {
...
function normalizeAddressBookId(value) {
...
function keyFor(scope, addressBookId) {
  return `${scope.tenantId}\u0000${scope.userId}\u0000${addressBookId}`;
}

function scopeFingerprint(scope, addressBookId) {
...
function makeSyncToken(scope, addressBookId, version) {
  return `${CARD_DAV_SYNC_TOKEN_PREFIX}${scopeFingerprint(scope, addressBookId)}:${version}`;
}

function parseSyncToken(token, scope, addressBookId) {
...
function assertIfMatch(ifMatch, currentEtag, resourceName) {
...
function readVCardInput(options) {
...
function unescapeVCardText(value) {
  return value.replaceAll(/\\([nNrR,;\\])/gu, (_match, escaped) => {
    if (escaped.toLowerCase() === 'n' || escaped.toLowerCase() === 'r') return '\n';
    return escaped;
  });
}

function validateVCardText(value) {
...
export function validateVCardObject(vCard) {
...
function contactMetadata(contact) {
  return freezeObject({
    href: contact.href,
    uid: contact.uid,
...
function publicAddressBook(state, scope) {
  return freezeObject({
...
export class CardDavStore {
  ...
  #getState(scopeInput, addressBookIdInput) {
  ...
  createAddressBook({ scope: scopeInput, addressBookId: addressBookIdInput, displayName, description = '', color = null } = {}) {
    const scope = normalizeScope(scopeInput);
    const addressBookId = normalizeAddressBookId(addressBookIdInput);
  ...
  getAddressBook({ scope: scopeInput, addressBookId: addressBookIdInput } = {}) {
    const { scope, state } = this.#getState(scopeInput, addressBookIdInput);
    return publicAddressBook(state, scope);
  }

  listAddressBooks({ scope: scopeInput } = {}) {
    const scope = normalizeScope(scopeInput);
    const result = [];
  ...
  deleteAddressBook({ scope: scopeInput, addressBookId: addressBookIdInput, ifMatch } = {}) {
  ...
  #touch(state, scope, addressBookId) {
    state.revision += 1;
  ...
  #buildContact({ state, scope, addressBookId, href, parsed, now }) {
  ...
  #record(state, scope, addressBookId, contact, operation, etag = null) {
  ...
  createContact({ scope: scopeInput, addressBookId: addressBookIdInput, href: hrefInput, ifNoneMatch, ...options } = {}) {
    const { scope, addressBookId, state } = this.#getState(scopeInput, addressBookIdInput);
  ...
  getContact({ scope: scopeInput, addressBookId: addressBookIdInput, href: hrefInput } = {}) {
  ...
  getContactMetadata({ scope: scopeInput, addressBookId: addressBookIdInput, href: hrefInput } = {}) {
    const { state } = this.#getState(scopeInput, addressBookIdInput);
    const href = normalizeHref(hrefInput);
    const contact = state.contacts.get(href);
    if (contact === undefined) {
  ...
  listContacts({ scope: scopeInput, addressBookId: addressBookIdInput } = {}) {
    const { state } = this.#getState(scopeInput, addressBookIdInput);
  ...
  putContact({ scope: scopeInput, addressBookId: addressBookIdInput, href: hrefInput, ifMatch, ifNoneMatch, ...options } = {}) {
    const { scope, addressBookId, state } = this.#getState(scopeInput, addressBookIdInput);
    const parsed = validateVCardText(readVCardInput(options));
  ...
  upsertContact(options = {}) {
    return this.putContact(options);
  }

  deleteContact({ scope: scopeInput, addressBookId: addressBookIdInput, href: hrefInput, ifMatch } = {}) {
    const { scope, addressBookId, state } = this.#getState(scopeInput, addressBookIdInput);
    const href = normalizeHref(hrefInput);
    const contact = state.contacts.get(href);
    if (contact === undefined) {
  ...
  syncCollection({ scope: scopeInput, addressBookId: addressBookIdInput, syncToken: suppliedToken } = {}) {
    const { scope, addressBookId, state } = this.#getState(scopeInput, addressBookIdInput);
    ...
    return freezeObject({
      ...
      changes: Object.freeze(state.changes
        .filter((change) => change.version > fromVersion)
        .map((change) => publicChange(change))),
    });
  }
   ...
   */
  exportAddressBookMetadata({ scope: scopeInput, addressBookId: addressBookIdInput } = {}) {
    const { scope, addressBookId, state } = this.#getState(scopeInput, addressBookIdInput);
...
export const createCardDavStore = (options = {}) => new CardDavStore(options);
